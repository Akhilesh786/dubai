             ...
             <?php include "navbar.php"; ?>
              ...
              <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Work Listing</h1>
                    </div>
                            ...
                            <div class="card shadow mb-4">
								...
								<table class="table table-bordered dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
                                    <thead>
										...
										<th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: auto">Title</th>
										<th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" style="width: auto">Services</th>
												...
												<?php
												 $i=0;
												$stmtuser = $dbh->query("SELECT * FROM `work` ORDER BY `work`.`id` DESC");
												while($row_work=$stmtuser->fetch()) {
												 ?>						
                                              <tbody>
						                        <tr role="row" class="odd">
                                                <td class="sorting_1"><?php echo ++$i; ?></td>
                                                <td><?php echo $row_work['work_title'];?></td>
                                                <td><?php $single_arr = array();
												$stmt1 = $dbh->query("SELECT * FROM `work_cate` WHERE work_id IN(".$row_work['work_service'].")");
												while ($row1= $stmt1 -> fetch(PDO::FETCH_ASSOC)) {
													$single_arr[] = $row1['name'];       
												}
											 echo	$service=implode(', ', $single_arr);?></td>
                                               
                                                <td>

														  
										<a href="edit-work.php?id=<?php echo $row_work['id'];?>" class="btn btn-primary btn-user btn-block">Edit</a>
                                               
											   </td>
											   <td>
													
										<a href="add/delete_work.php?id=<?php echo $row_work['id'];?>" class="btn btn-danger btn-user btn-block">Delete</a>
